...
// Supabase config (anon key — safe to expose; insert/select are gated by RLS)
const SUPABASE_URL = 'https://xgqitkqhruitdviegnqb.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhncWl0a3FocnVpdGR2aWVnbnFiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcxNjY5NzIsImV4cCI6MjA5Mjc0Mjk3Mn0.c-_SS6_y4aNKlnzTu1UVvOyDZWh6PmuKkjynJ2P5AjQ';

// Per-device id (lets us update one row per device instead of spamming)
function getClientId() {
    try {
        let id = localStorage.getItem('flapEmonadClientId');
        if (!id) {
            id = (crypto.randomUUID && crypto.randomUUID()) ||
                 (Date.now().toString(36) + Math.random().toString(36).slice(2, 10));
            localStorage.setItem('flapEmonadClientId', id);
        }
        return id;
    } catch (e) {
...
function getSavedName() {
    try { return localStorage.getItem('flapEmonadPlayerName') || ''; } catch (e) { return ''; }
}
function saveName(name) {
    try { localStorage.setItem('flapEmonadPlayerName', name); } catch (e) {}
}

function sanitizeName(raw) {
    return (raw || '').replace(/[^A-Za-z0-9 _-]/g, '').trim().slice(0, 12);
}

// XMLHttpRequest wrapper — bypasses wallet extensions that hook window.fetch
...
async function submitScoreToBoard(name, scoreVal) {
    ...
    const body = JSON.stringify({ name, score: scoreVal, client_id: clientId });
    await xhrRequest('POST', url, headers, body);
    return true;
}

async function fetchRankFor(myName) {
    const targetKey = (myName || '').toLowerCase();
    const url = SUPABASE_URL +
        '/rest/v1/scores?select=name,score&order=score.desc,created.asc&limit=500';
    const headers = {
        'apikey': SUPABASE_KEY,
        'Authorization': 'Bearer ' + SUPABASE_KEY
    };
    try {
        const res = await xhrRequest('GET', url, headers, null);
...
// Home button: navigate back to emo site
if (homeBtn) {
    homeBtn.addEventListener('click', () => {
        if (typeof chiptunePlayer !== 'undefined') chiptunePlayer.playClick();
        navigateWithTransition('../emo.html');
    });
}
...
if (submitScoreBtn) {
    submitScoreBtn.addEventListener('click', async () => {
        ...
        if (!name) {
            ...
            submitStatus.textContent = 'Enter a name (1-12 chars)';
        ...
        try {
            await submitScoreToBoard(name, deathCertificate.finalScore);
            const rank = await fetchRankFor(name);
            submitStatus.textContent = rank
                ? 'You ranked #' + rank + '!'
                : 'Submitted!';
...
function showGameOver() {
    ...
    // Reset submit UI each death; always show it so the player can submit any run
    if (submitBlock && submitScoreBtn && submitStatus && playerNameInput) {
        ...
        submitScoreBtn.style.display = 'inline-flex';
...
function resetGame() {
    ...
    // Clear lingering death effects (in case player restarts mid-effect)
...
async function init() {
    ...
    try {
        ...
        // Stop the loading bird flap so it doesn't run forever if user never taps PLAY
        if (window._loadingBirdInterval) {
            clearInterval(window._loadingBirdInterval);
            window._loadingBirdInterval = null;
        }
